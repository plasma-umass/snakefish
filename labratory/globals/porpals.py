...
class Example():
    ...
    def porpal_merge(self, old_self, joined):
        ...
        self.list.extend(joined.list)
        self.dict.update(joined.dict)

### Example pre function
def pre(wrapped, instance, args, kwargs):
...
def aset(instance, type, name, value):
    print('setting at ', name, ' ', value)
    return value
...
# Objects produced here will have their own log at _porpal_log.
...
class PorpalLogger(PorpalSigniture):
	def __init__(self, obj, fpre, fpost, aset, olog):
		...
		super().__init__(obj)
		super().__setattr__('_porpal_log', olog)
	...
	def __setattr__(self, name, value):
		if porpals_record:
			...
			if ret is not None:
				super().__getattribute__('_porpal_log').append(ret)
