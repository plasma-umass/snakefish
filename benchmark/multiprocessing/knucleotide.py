...
def display(f, results, display_list, sort=False, relative=False, end='\n'):
    lines = [
    ...
    for k_nucleotide, (frequency, n) in lines:
        if relative:
            f.write("{0} {1:.3f}\n".format(k_nucleotide, frequency * 100. / n))
        else:
            f.write("{1}\t{0}\n".format(k_nucleotide, frequency))
    f.write(end)
...
def main():
    ...
    with open("bench_output-fasta_mp.txt", mode="rb") as f:
    ...
    if n == 1:
        ...
        results = wrappers.starmap(lean_call(count_frequencies), lean_jobs)
        results = list(chain(*results))

    with open("bench_output-knucleotide_mp.txt", mode="w") as f:
        display(f, results, display_list(mono_nucleotides), relative=True, sort=True)
        display(f, results, display_list(di_nucleotides), relative=True, sort=True)
        display(f, results, display_list(k_nucleotides), end='')
