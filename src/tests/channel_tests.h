...
TEST(ChannelTest, ReadWriteNoWrapping) {
  ...
  ASSERT_EQ((channel.ref_cnt)->load(), 1);
  ASSERT_EQ((channel.local_ref_cnt)->load(), 1);
...
TEST(ChannelTest, IpcReadWrite) {
  ...
  if (result == 0) {
    ...
    channel.send_bytes(bytes.get_ptr(), TEST_CAPACITY);
    ASSERT_NE(channel.shared_mem, nullptr);
    ASSERT_EQ((channel.ref_cnt)->load(), 2);
    ASSERT_EQ((channel.local_ref_cnt)->load(), 1);
    ASSERT_EQ((channel.start)->load(), 0);
    ASSERT_EQ((channel.end)->load(), 0);
    ASSERT_EQ((channel.full)->load(), true);
...
TEST(ChannelTest, MultiProcessSharing) {
  ...
  if (result < 0) {
    ...
    ASSERT_EQ((channel.ref_cnt)->load(), 3);
    ASSERT_EQ((channel.local_ref_cnt)->load(), 2);
    ...
    if (result < 0) {
      ...
      // grandchild
      ASSERT_EQ((channel.ref_cnt)->load(), 5);
      ASSERT_EQ((channel.local_ref_cnt)->load(), 2);
